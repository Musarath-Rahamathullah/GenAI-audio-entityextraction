...
TRANSCRIBE_INPUT=config_file["transcribe-input-bucket"]
TRANSCRIBE_OUTPUT=config_file["transcribe-output-bucket"]
...
def store_transcribe_text_s3(txt_file_uri,text):
    s3_path_split = txt_file_uri.split('/')

    bucket_name = s3_path_split[2]
    key_name = '/'.join(s3_path_split[3:])
    S3.put_object(Body=text, Bucket=bucket_name, Key=key_name)
...
def transcribe_file(file_uri):
    ...
    try:
        ...
        transcribe_client.start_transcription_job(
            ...
            MediaFormat = 'wav',
            LanguageCode = 'en-US',
            OutputBucketName=TRANSCRIBE_OUTPUT,
            OutputKey=f"transcripts/{job_name.split('/')[-1]}.json"
        )
        ...
        while max_tries > 0:
            ...
            if job_status in ['COMPLETED', 'FAILED']:
                ...
                time.sleep(5)
        response = S3.get_object(Bucket=TRANSCRIBE_OUTPUT, Key=f"transcripts/{job_name.split('/')[-1]}.json")
        response = json.loads(response['Body'].read().decode('utf-8'))
        response = response['results']['transcripts']
        text = '\n'.join([t['transcript'] for t in response])
        
        txt_file_uri = file_uri.replace('wav','txt',-1)
        store_transcribe_text_s3(txt_file_uri,text)
        
        doc_name = get_filename_from_s3_uri(file_uri)
        result_string+=f"<{doc_name}>\n{text}\n</{doc_name}>\n"
        

    except Exception as e:
        # Get the original function arguments from the Future object
        error = {'file': file_uri, 'error': str(e)}
        # errors.append(error)         
...
def handle_doc_upload_or_s3(file):
    ...
    if  ext.lower() in [".pdf", ".png", ".jpg",".tif",".jpeg"]:   
        ...
        print("handle_doc_upload_or_s3 file:",file)
        file = file.replace('.wav','.txt',1)
        obj=get_s3_obj_from_bucket_(file)
        content = obj['Body'].read()
    else:
        obj=get_s3_obj_from_bucket_(file)
...
def put_obj_in_s3_bucket_(docs):
    ...
    file_path=f"{TRANSCRIBE_INPUT}/{file_name}"
...
def query_llm(params, handler):
    ...
    if params['upload_doc']:  
        ...
        for ids,docs in enumerate(params['upload_doc']):
            ...
            # if extensions.lower() in [".xlsx"]
            #     result_string = extract_excel_data(file_name)
            #     doc+= result_string
            #     continue
        ...
        result_string = ""
        for each_file in doc_path:
            print("each file:",each_file)
            
            errors, text = transcribe_file(each_file)
            result_string+=text             
        if errors:
            st.error(errors)
        print("transcribe:",result_string)
        # doc+= result_string
        user_prompt = f"""{doc}
            <!-- This is the transcription you will work on. -->
            <content_to_extract>
            {result_string}
            </content_to_extract>
            
            <!-- This is the dictionary you will use to look up information in transcription messages.  It tells you their entity name, description, rules_for_use, and  json_response_format  -->
            <transcription_dictionary>
                <entries>
                    <entry>
                        ...
                        <rules_for_use>
                        1. If unable to determine the NAME, fill NULL
                        </rules_for_use>
                    </entry>
                    <entry>
                        <name>DOB</name>
                        <description>Date of Birth of the patient</description>
                        <rules_for_use>
                        1. If unable to determine the DOB, fill NULL
                        </rules_for_use>
                    </entry>
                    <entry>
                        <name>MESSAGE</name>
                        <description>Reason for the patient calling</description>
                        <rules_for_use>
                        1. If unable to determine the transcription MESSAGE, fill NULL
                        </rules_for_use>
                    </entry>
            ...
            <task_guidance>
            1. If you encounter a multiple transcripts inside <content_to_extract></content_to_extract> XML tag, with each transcript seperated by <doc_name></doc_name>                XML tag. Then generate the response for each  transcript as seperate json object
            2. You must adhere to the json_response_format provided to you in the <json_response_format></json_response_format> XML tags.
            3. Take a deep breath and learn the rules given to you in the <rules_for_use></rules> XML tags.
            4. Refer to and adhere to the task_guidance provided to you in the <task_guidance></task_guidance> XML tags.
            5. Do NOT include preamble.Directly give response.
            </task_guidance>

            <!-- This is the format you must adhere to in your response. -->
            <json_response_format>
            {{ "[REPLACE WITH YOUR DOC NAME]" :
                {{  "NAME" : "[REPLACE WITH YOUR GENERATED NAME VALUE]",
                    "DOB": "[REPLACE WITH YOUR GENERATED DOB VALUE]",
                    "MESSAGE": "[REPLACE WITH YOUR GENERATED MESSAGE VALUE]",
                }}
            }}
            </json_response_format>
            ...
            <tasks>
            {params['question']}
            Extract the information from each transcript using the information provided to you in the <content_to_extract></content_to_extract> XML tags and double check your work. Do NOT include preamble.Directly give questions.
            </tasks>
            """
    ...
    response=_invoke_bedrock_with_retries(params,current_chat, chat_template, user_prompt, model, image_path, handler)
...
def app_sidebar():
    with st.sidebar:   
        ...
        file = st.file_uploader('Upload a document', accept_multiple_files=True, help="wav format supported") 
        ...
        with st.expander("Architecture"):
            st.write("In this demo, we demonstrate how to implement a Entity extraction from Audio transcript by using LLMs from Amazon Bedrock using GenAI application.")
